...
export default function SteelCuttingCalculator() {
  ...
  // 切断仕様一覧（文字列型で管理）
  const [cutSpecs, setCutSpecs] = useState<CutSpec[]>([
    { length: "", quantity: "" },
  ]);
  ...
  const calculateCuttingPlan = () => {
    if (!standardLength || !bladeThickness) {
      window.alert("「定尺の長さ」もしくは「切断の刃幅」が未入力です。");
    }
    ...
    // 長さが長い順にソート
    const sortedCutSpecs = [...numericCutSpecs].sort(
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="grid gap-4">
        ...
        <div>
          ...
          <Input
            ...
            onChange={handleNumericInput(setStandardLength)}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={handleNumericInput(setBladeThickness)}
          />
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2 mb-5">
            ...
            <Button
              className="bg-[#458af7] hover:bg-[#3365b5]"
          ...
          {cutSpecs.map((spec, index) => (
            <CutSpecInput
              ...
              index={index}
              spec={spec}
              onChange={updateCutSpec}
              onDelete={removeCutSpec}
            />
          ))}
        </div>
      </div>
      <div className="print-result">
        <CuttingPlanResult
          result={result}
          standardLength={parseInt(standardLength, 10) || 0}
        />
      </div>
    </div>
  );
